...
export class PrevInculpadoService {

  other_header: any;
  api: any;
  
  constructor(private http: HttpClient) {   
    this.api = environment.URL + "PrevInculpado";
  }
  ...
  doFilter(criterio: any){
    this.other_header = this.other_header;
    const ruta = this.api+'/'+'filterInculpado/';
    return this.http
      .get(ruta + criterio)
      .toPromise()
      .catch((err) => {
  ...
  doFilterPlanillaHDInc(fecha1?: any, fecha2?: any, departamento?: any, localidad?: any, zonaMetro?: any, dnpc?: boolean) {
    this.other_header = this.other_header;
    const ruta = this.api + '/' + 'filterPlanillaHechosDLInculpado/';
    return this.http
